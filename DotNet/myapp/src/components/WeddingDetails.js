...
const WeddingDetails = () => {
  ...
  const [formData, setFormData] = useState({
    ...
    "userId": id,
    "facilityId": facilityDetails?.facilityId || -1,
    "categoryId": facilityDetails?.categoryId || -1
  });
  ...
  function getUserData(id) {
    try {
      axios.get(`http://localhost:5078/api/User/${parseInt(id)}`)
      .then(response => {
  ...
  function getFacilityData(){
    try {
      axios.get(`http://localhost:5078/api/Facilities/GetByFacilityId/${facilityId}`)
      .then(response => {
        const data = response.data;
        console.log("Category Details: ", data);
        setFacilityDetails(data); // Set userDetails only after data is fetched
        ...
        const id = userId;
        console.log("SamiCutie",id);
        const price = data.basePrice - ((data.basePrice * data.discount) / 100);
        console.log(totalPrice, discountAmount);
        setFormData({ ...formData, ["facilityId"]: data.facilityId, ["categoryId"]: data.categoryId, ["totalPrice"]: price});
      });
  ...
  const handlePayment = (event) => {
    ...
    try {
      axios.post(`http://localhost:5078/add`, formData)
      .then(response => {
      ...
      <Row>
        <Col md={6}>
          <Card>
            ...
            />
            <Card.Body>
              <Card.Title>{facilityDetails?.title || ""}</Card.Title>
              <Card.Text>
                <strong>Description:</strong> {facilityDetails?.description || ""}
              </Card.Text>
              <Card.Text>
                <strong> Price:</strong> ₹ {facilityDetails?.basePrice || ""}
              </Card.Text>
              <Card.Text>
                <strong>Discount:</strong> {facilityDetails?.discount || ""}%
              </Card.Text>
              <Card.Text>
                <strong>Rating:</strong> {facilityDetails?.rating || ""}
                <span></span>
              </Card.Text>
          ...
          {!showBilling ? (
            ...
            <Card>
              <Card.Body>
                ...
                <Card.Text>
                  <strong>Base Price:</strong> ₹ {facilityDetails?.basePrice.toLocaleString() || ""}
                </Card.Text>
                <Card.Text>
                  <strong>Discount:</strong> {facilityDetails.discount}% (₹{" "}
